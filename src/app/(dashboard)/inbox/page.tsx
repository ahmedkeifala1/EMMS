import { auth } from "@/lib/auth";
import { prisma } from "@/lib/prisma";
import type { Prisma } from "@/generated/prisma/client";
import { redirect } from "next/navigation";
import { Suspense } from "react";
import { MemoCard } from "@/components/memos/MemoCard";
import { MemoFilters } from "@/components/memos/MemoFilters";
import { Inbox, ChevronLeft, ChevronRight } from "lucide-react";
import Link from "next/link";

interface SearchParams {
  search?: string;
  priority?: string;
  audienceType?: string;
  dateFrom?: string;
  dateTo?: string;
  senderSearch?: string;
  page?: string;
}

async function getInboxMemos(
  userId: string,
  params: SearchParams,
  archived = false
) {
  const page = parseInt(params.page || "1");
  const limit = 20;
  const skip = (page - 1) * limit;

  const andClauses: Prisma.MemoWhereInput[] = [];
  if (params.search) {
    andClauses.push({
      OR: [
        { subject: { contains: params.search, mode: "insensitive" } },
        { referenceNumber: { contains: params.search, mode: "insensitive" } },
        { sender: { fullName: { contains: params.search, mode: "insensitive" } } },
        { body: { contains: params.search, mode: "insensitive" } },
      ],
    });
  }
  if (params.senderSearch) {
    andClauses.push({ sender: { fullName: { contains: params.senderSearch, mode: "insensitive" } } });
  }
  if (params.dateFrom) {
    andClauses.push({ sentAt: { gte: new Date(params.dateFrom) } });
  }
  if (params.dateTo) {
    const to = new Date(params.dateTo);
    to.setHours(23, 59, 59, 999);
    andClauses.push({ sentAt: { lte: to } });
  }

  const where: Prisma.MemoWhereInput = {
    AND: [
      {
        status: { not: "draft" },
        recipients: { some: { userId, receiptType: "to", isArchived: archived } },
        ...(params.priority ? { priority: params.priority as never } : {}),
        ...(params.audienceType ? { audienceType: params.audienceType as never } : {}),
      },
      ...andClauses,
    ],
  };

  const [memos, total] = await Promise.all([
    prisma.memo.findMany({
      where,
      orderBy: [{ priority: "asc" }, { sentAt: "desc" }],
      skip,
      take: limit,
      include: {
        sender: { select: { fullName: true, department: { select: { name: true } } } },
        recipients: {
          where: { userId },
          select: { receiptType: true, acknowledgedAt: true, readAt: true },
        },
      },
    }),
    prisma.memo.count({ where }),
  ]);

  return { memos, total, page, pages: Math.ceil(total / limit) };
}

export default async function InboxPage({
  searchParams,
}: {
  searchParams: Promise<SearchParams>;
}) {
  const session = await auth();
  if (!session) redirect("/login");

  const params = await searchParams;
  const { memos, total, page, pages } = await getInboxMemos(session.user.id, params);

  return (
    <div className="max-w-4xl space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Inbox className="h-5 w-5 text-[#003087]" />
          <h2 className="text-lg font-bold text-gray-900">Inbox</h2>
          <span className="ml-1 rounded-full bg-[#003087] px-2 py-0.5 text-xs font-bold text-white">
            {total}
          </span>
        </div>
        <Link
          href="/archive"
          className="text-xs text-gray-500 hover:text-[#003087]"
        >
          View Archive →
        </Link>
      </div>

      <Suspense>
        <MemoFilters />
      </Suspense>

      {memos.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-gray-300 bg-white py-16 text-center">
          <Inbox className="mx-auto h-10 w-10 text-gray-300" />
          <p className="mt-3 text-sm font-medium text-gray-500">No memos in your inbox</p>
          <p className="mt-1 text-xs text-gray-400">New memos will appear here when sent to you.</p>
        </div>
      ) : (
        <div className="space-y-2">
          {memos.map((memo) => (
            <MemoCard key={memo.id} memo={memo} viewAs="recipient" />
          ))}
        </div>
      )}

      {/* Pagination */}
      {pages > 1 && (
        <div className="flex items-center justify-between pt-2">
          <p className="text-sm text-gray-500">
            Page {page} of {pages} · {total} total
          </p>
          <div className="flex gap-2">
            {page > 1 && (
              <Link
                href={`?page=${page - 1}`}
                className="flex items-center gap-1 rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-sm hover:bg-gray-50"
              >
                <ChevronLeft className="h-4 w-4" /> Prev
              </Link>
            )}
            {page < pages && (
              <Link
                href={`?page=${page + 1}`}
                className="flex items-center gap-1 rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-sm hover:bg-gray-50"
              >
                Next <ChevronRight className="h-4 w-4" />
              </Link>
            )}
          </div>
        </div>
      )}
    </div>
  );
}
